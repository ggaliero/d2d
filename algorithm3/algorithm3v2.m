 ...
 for i=1:m
     ...
     num_ch(i) = num_changes(pos_no_rsc);
    ...
    %  [num_c, num_w, mse] = performance(data, thr, pos_yes_rsc, pos_no_rsc);
     ...
     [num_c(i), num_w(i), pos_w] = performance2(data(i,:), thr, pos_yes_rsc, pos_no_rsc);

     pos_w = sort(pos_w);

     mse(i) = mean((data(i,pos_w) - thr).^2);
     me(i) = mean(abs(data(i,pos_w) - thr));
 ...
 num_ch_avg = mean(num_ch);
 num_c_avg = mean(num_c);
 num_w_avg = mean(num_w);
 
 
 str1 = ['Accuracy of the algorithm: ', num2str(accuracy_avg)];
 str2 = ['Number of mode changes: ', num2str(num_ch_avg)];
 str3 = ['Number of signal samps correct: ', num2str(num_c_avg)];
 str4 = ['Number of signal samps wrong: ', num2str(num_w_avg)];
 str5 = ['Mean Square Error: ', num2str(mse_avg)];
 str6 = ['Mean Error:', num2str(me_avg)];
 disp(str1);
 disp(str2);
 disp(str3);
 disp(str4);
